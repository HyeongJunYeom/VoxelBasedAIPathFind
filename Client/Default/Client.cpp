                     ...
                     _In_ int       nCmdShow)
{
	...
	while (true)
	{
		...
		fTimeAcc += fminf(pGameInstance->Compute_TimeDelta(TEXT("Timer_Default")), 1.f / 60.f);
...
{
	if (nullptr != CGameInstance::Get_Instance() &&
		CGameInstance::Get_Instance()->ImGui_ImplWin32_WndProcHandler(hWnd, message, wParam, lParam))
	{
		return true;
	}
